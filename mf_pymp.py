...
def matrix_factorization(X,P,Q,K,steps,alpha,beta, thr):
    Q = Q.T
    r = []
    for step in range(steps):
        ...
        #update Q
        with pymp.Parallel(thr) as q:
            for j in q.range(0,X.shape[1]):
...
#main function
def main(X,K,thr,stepnum):
    N= X.shape[0] #no of users
    M = X.shape[1] #no of movies
    P = np.random.rand(N,K)
    P = np2pymp(P, thr) #P: an initial matrix of dimension N x K, where is n is no of users and k is hidden latent features

    Q = np.random.rand(M,K)
    Q = np2pymp(Q, thr) #Q : an initial matrix of dimension M x K, where M is no of movies and K is hidden latent features
    ...
    beta = float(0.02)
    start = time.time()
    estimated_P, estimated_Q = matrix_factorization(X,P,Q,K,steps,alpha,beta, thr) 
    CostTime = round(time.time() - start, 3)
    print('[matrix_factorization_pymp] Using {} threads, {} steps: '.format(thr, stepnum),str(timedelta(seconds=CostTime)))
    filename = 'time_mf_pymp.txt'
    if filename not in os.listdir():
        f = open(filename,'w')
    with open(filename, 'a') as the_file:
        the_file.write(str(thr)+','+str(stepnum) + ',' + str(CostTime)+'\n')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 6:
        ...
        modeled_X = main(numpy_arr,no_of_features,thr,stepnum) #comment out if using train and test below
        '''
        ###train & test###
        #split data
        train, test = eval.split_train_test(numpy_arr)
        #main function
        modeled_X = main(train,no_of_features,thr,stepnum)
