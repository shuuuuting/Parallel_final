...
@ray.remote
def updateP(X,P,Q,K,alpha,beta,idx,num):   
    out = np.array_split(P, num)[idx].copy()
    splitX = np.array_split(X, num)[idx]
    for i in range(out.shape[0]): #i是某個worker自己計算的，所以當動到p[i][k]的時候，不會影響到別的worker會用到的p[i,:]
        for j in range(X.shape[1]): 
            if splitX[i][j] > 0 :
                eij = splitX[i][j] - np.dot(out[i,:],Q[:,j]) #(r-PQ) 
                for k in range(K):
                    out[i][k] = out[i][k] + alpha * ( 2 * eij * Q[k][j] - (beta * out[i][k]))
    #print(out)
    return out

@ray.remote
def updateQ(X,P,Q,K,alpha,beta,idx,num):
    part = (X.shape[1]+idx)//num
    start = 0
    for idc in range(idx):
        start += (X.shape[1]+idc)//num
    out = Q[:,start:start+part].copy()

    for j in range(out.shape[1]):
        for i in range(X.shape[0]):
            if X[i][start+j] > 0 :
                eij = X[i][start+j] - np.dot(P[i,:],out[:,j]) #(r-PQ)
                for k in range(K):
                    out[k][j] = out[k][j] + alpha * (2 * eij * P[i][k] - (beta * out[k][j]))
    return out

# non negative regulaized matrix factorization implemention
def matrix_factorization(X,P,Q,K,steps,alpha,beta,num_cpus):
    rmse_list = []
    ...
    for step in range(steps):
        ...
        #update P
        result_ids_p = [updateP.remote(X_id,P_id,Q_id,K,alpha,beta,i,num_cpus) for i in range(num_cpus)]
        results_p = ray.get(result_ids_p)
        P = np.vstack(results_p)

        #update Q
        P_id = ray.put(P)
        result_ids_q = [updateQ.remote(X_id,P_id,Q_id,K,alpha,beta,i,num_cpus) for i in range(num_cpus)]
        results_q = ray.get(result_ids_q)
        Q = np.hstack(results_q)
...
def main(X,K,num_cpus,stepnum):
    ...
    beta = float(0.02)
    start = time.time()
    estimated_P, estimated_Q = matrix_factorization(X,P,Q,K,stepnum,alpha,beta,num_cpus) 
    CostTime = round(time.time() - start, 3)
    print('[matrix_factorization_ray] Using {} cpus, {} steps: '.format(num_cpus, stepnum),str(timedelta(seconds=CostTime)))
    filename = 'time_mf_ray.txt'
    if filename not in os.listdir():
        f = open(filename,'w')
    with open(filename, 'a') as the_file:
        the_file.write(str(num_cpus)+','+str(stepnum) + ',' + str(CostTime)+'\n')
    '''
    #estimated_P, estimated_Q, err_list = matrix_factorization(X,P,Q,K,stepnum,alpha,beta,num_cpus)     
    #Plot error process
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 6:
        ...
        modeled_X = main(numpy_arr,no_of_features,num_cpus,stepnum) #comment out if using train and test below
        '''
        ###train & test###
        #split data
        train, test = eval.split_train_test(numpy_arr)
        #main function
        modeled_X = main(train,no_of_features,num_cpus,stepnum)
