...
def top_25_recommended_movies(pred_rating_file,users,unrated_movies_per_user,movies_mapping_names,movie_mapping_id, thr):
...
#main method
def recommend_movies_for_users(orig_rating_file,pred_rating_file,movies_file,users, thr):
    ...
    start = time.time()
    top_25_recommended_movies(predicted_rating_numpy_array,users,dict_with_unrated_movies_users,movies_mapping_names,movie_mapping_id, thr)
    CostTime = round(time.time() - start, 3)
    filename = 'time_rec_pymp.txt'
    if filename not in os.listdir():
        f = open(filename,'w')
    with open(filename, 'a') as the_file:
        the_file.write(str(thr) + ','+str(usernum)+',' + str(CostTime)+'\n')
...
if __name__ == '__main__':
    if len(sys.argv) == 6:
        ...
        start = time.time()
        recommend_movies_for_users(orig_rating_file,pred_rating_file,movies_file,users, thr)
        CostTime = round(time.time() - start, 3)
        print('[recommend_movies_for_users]Using {} threads, {} users: '.format(thr, usernum),str(timedelta(seconds=CostTime)))
