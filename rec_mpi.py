...
def top_25_recommended_movies(pred_rating_file,users,unrated_movies_per_user,movies_mapping_names,movie_mapping_id):
    ...
    comm = MPI.COMM_WORLD
    rank = comm.rank
    splitusers = np.array_split(users, comm.Get_size())
    for user in splitusers[rank]: #for user in users:
...
#main method
def recommend_movies_for_users(orig_rating_file,pred_rating_file,movies_file,users,size,comm):
    ...
    if comm.rank==0:
        ...
        filename = 'time_rec_mpi.txt'
        if filename not in os.listdir():
            f = open(filename,'w')
        with open(filename, 'a') as the_file:
            the_file.write(str(size) + ','+str(usernum)+',' + str(CostTime)+'\n')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 5:
        ...
        start = time.time()
        recommend_movies_for_users(orig_rating_file,pred_rating_file,movies_file,users,size,comm)
        if comm.rank==0:
            costTime = round(time.time() - start, 3)
            print('[recommend_movies_for_users_mpi]Using {} processors, {} users: '.format(size, usernum),str(timedelta(seconds=costTime)))
