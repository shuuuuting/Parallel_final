...
if __name__ == '__main__':
    if len(sys.argv) == 5:
        ...
        print('[recommend_movies_for_users]Using, {} users: '.format(usernum),str(timedelta(seconds=CostTime)))
        filename = 'time_rec_serial.txt'
        if filename not in os.listdir():
           f = open(filename,'w')
        with open(filename, 'a') as the_file:
           the_file.write(str(usernum)+',' + str(CostTime)+'\n')
